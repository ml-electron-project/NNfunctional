...
#loading NN weights
model = Net()
model.load_state_dict(torch.load("NNGGA",map_location='cpu'))
...
def eval_xc(xc_code, rho, spin, relativity=0, deriv=2, verbose=None):
    ...
    rho0=np.concatenate((rho01.reshape((-1,1)),rho02.reshape((-1,1)),gamma1.reshape((-1,1)),gamma12.reshape((-1,1)),gamma2.reshape((-1,1))),axis=1)
